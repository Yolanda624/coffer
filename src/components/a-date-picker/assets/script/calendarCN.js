...
const creatLenArr = function (year, month, len, start) {
  const arr = []
  start = start || 0
  if (len < 1) return arr
  let k = start
...
const formatDayD4 = function (month, day) {
  month = month + 1
...
function getLunarYearDays(year) {
  ...
  const monthDataArr = monthData.split('').map(item => parseInt(item))
...
function getLunarDateByBetween(year, between) {
  const lunarYearDays = getLunarYearDays(year)
...
function getLunarByBetween(year, month, day) {
  ...
  const between = getDaysBetweenSolar(year, zenMonth - 1, zenDay, year, month, day)
  if (between === 0) { //正月初一
    return [year, 0, 1]
  } else {
...
function getTerm(y, n) {
  const offDate = new Date((31556925974.7 * (y - 1890) + termInfo[n] * 60000) + Date.UTC(1890, 0, 5, 16, 2, 31))
...
function getYearZodiac(year) {
  const num = year - 1890 + 25 //参考干支纪年的计算，生肖对应地支
...
function lunarToSolar(_year, _month, _day) {
  const inputDate = formatDate(_year, _month, _day)
  if (inputDate.error) return inputDate
  const year = inputDate.year
  const month = inputDate.month
  const day = inputDate.day

  const between = getDaysBetweenZheng(year, month, day) //离正月初一的天数
  const yearData = lunarInfo[year - minYear]
  const zenMonth = yearData[1]
  const zenDay = yearData[2]

  let offDate = new Date(year, zenMonth - 1, zenDay).getTime() + between * 86400000
...
function solarToLunar(_year, _month, _day) {
  const inputDate = formatDate(_year, _month, _day, minYear)
  if (inputDate.error) return inputDate
  const year = inputDate.year
  const month = inputDate.month
  const day = inputDate.day
  ...
  //立春日期
  const term2 = cacheUtil.get('term2') ? cacheUtil.get('term2') : cacheUtil.set('term2', getTerm(year, 2))
  //二十四节气
  const termList = cacheUtil.get('termList') ? cacheUtil.get('termList') : cacheUtil.set('termList', getYearTerm(year))

  const firstTerm = getTerm(year, month * 2) //某月第一个节气开始日期
  const GanZhiYear = (month > 1 || month === 1 && day >= term2) ? year + 1 : year //干支所在年份
  const GanZhiMonth = day >= firstTerm ? month + 1 : month  //干支所在月份（以节气为界）

  const lunarDate = getLunarByBetween(year, month, day)
  const lunarLeapMonth = getLunarLeapYear(lunarDate[0])
  let lunarMonthName
  ...
  //农历节日判断
  let lunarFtv
  const lunarMonthDays = getLunarYearDays(lunarDate[0]).monthDays
  //除夕
...
function calendar(_year, _month, fill) {
  const inputDate = formatDate(_year, _month)
  if (inputDate.error) return inputDate
  const year = inputDate.year
  const month = inputDate.month

  const calendarData = solarCalendar(year, month + 1, fill)
  for (let i = 0; i < calendarData.monthData.length; i++) {
    const cData = calendarData.monthData[i]
    const lunarData = solarToLunar(cData.year, cData.month, cData.day)
...
function solarCalendar(_year, _month, fill) {
  ...
  let preMonthDays, preMonthData, nextMonthData
  ...
  if (fill) {
    if (res.firstDay > 0) { //前补
      ...
      const preMonth = month - 1 < 0 ? 11 : month - 1
    ...
    if (7 * 6 - res.monthData.length !== 0) { //后补
      ...
      const fillLen = 7 * 6 - res.monthData.length
